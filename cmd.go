...
func processMessageAsync(client *whatsmeow.Client, v *events.Message) {
	...
	// XP system: har message pe XP add karo (group messages only)
	if strings.Contains(v.Info.Chat.String(), "@g.us") {
		senderName := v.Info.PushName
		go addXP(v.Info.Sender.User, senderName)
	}
...
case "status":
   ...
   case "insult", "roast":
	...
	case "mute":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleMute(client, v)

	case "unmute":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleUnmute(client, v)

	case "lock":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleLock(client, v)

	case "unlock":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleUnlock(client, v)

	case "kickall":
		if !userIsOwner { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleKickAll(client, v)

	case "groupdesc":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleGroupDesc(client, v, fullArgs)

	case "grouplink":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleGroupLink(client, v, fullArgs)

	case "groupinfo":
		go handleGroupInfo(client, v)

	case "getpic":
		go handleGetPic(client, v, fullArgs)

	case "tagadmin":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleTagAdmin(client, v, fullArgs)

	case "poll":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handlePoll(client, v, fullArgs)

	case "setwelcome":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleSetWelcome(client, v, fullArgs)

	case "setgoodbye":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleSetGoodbye(client, v, fullArgs)

	case "requests":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleRequests(client, v)

	case "accept":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleAccept(client, v, fullArgs)

	case "reject":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleReject(client, v, fullArgs)

	case "opentime":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleOpenTime(client, v, fullArgs)

	case "closetime":
		if !userIsOwner && !isGroupAdmin(client, v) { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleCloseTime(client, v, fullArgs)

	// ── EXTRA TOOLS (from Zeno) ──────────────────────────────

	case "qr":
		react(client, v.Info.Chat, v.Info.ID, "🔲")
		go handleQR(client, v, fullArgs)

	case "ocr":
		react(client, v.Info.Chat, v.Info.ID, "🔍")
		go handleOCR(client, v)

	case "ttp":
		react(client, v.Info.Chat, v.Info.ID, "🎨")
		go handleTTP(client, v, fullArgs)

	case "encode":
		go handleEncode(client, v, fullArgs)

	case "decode":
		go handleDecode(client, v, fullArgs)

	case "dice":
		go handleDice(client, v)

	case "coin":
		go handleCoin(client, v)

	case "wiki", "wikipedia":
		go handleWiki(client, v, fullArgs)

	case "bc", "broadcast":
		if !userIsOwner { react(client, v.Info.Chat, v.Info.ID, "❌"); return }
		go handleBroadcast(client, v, fullArgs)

	case "rank":
		go handleRank(client, v)

	case "leaderboard", "lb", "top":
		go handleLeaderboard(client, v)

	case "fact":
		go handleFact(client, v)

	case "joke":
		go handleJoke(client, v)

	case "calc", "calculate":
		go handleCalc(client, v, fullArgs)
